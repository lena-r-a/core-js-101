...
function unbracketTag(str) {
  return str[0].replace('<', '') + str.substring(1, str.length - 1)
...
function getRectangleString(width, height) {
  ...
  const lasLine = `└${'─'.repeat(width - 2)}┘\n`;
  const otherLines = `│${' '.repeat(width - 2)}│\n`;
  return firstLine + otherLines.repeat(height - 2) + lasLine;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    result += outputStr[inputStr.indexOf(str[i])];
  }
  return result;
}
